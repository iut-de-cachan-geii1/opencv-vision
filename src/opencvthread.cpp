...
void OpenCVThread::run()
{
    if (_argc != 4)
    {
        std::cout << "usage: " << _argv[0] << " </path/to/haar_cascade> </path/to/csv.ext> </path/to/device id>\n";
    ...
    std::string fn_haar = std::string(_argv[1]);
    std::string fn_csv = std::string(_argv[2]);
    int deviceId = atoi(_argv[3]);
